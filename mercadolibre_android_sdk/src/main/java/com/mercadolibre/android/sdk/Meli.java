...
public final class Meli {
    ...
    private static boolean loadIdentity(@NonNull Context context) {
        ...
        if (meliIdentity != null && meliIdentity.getAccessToken() != null) {
            long tokenLifetime = meliIdentity.getAccessToken().getAccessTokenLifetime();
            if (System.currentTimeMillis() <= tokenLifetime)
                return true;
        }
        return false;
    }
